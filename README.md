...
`src/ui/theme.tsx` decides colours and faces from the device's appearance;
its palette, `src/ui/tokens.ts`, is generated by `npm run tokens` from
[testdata/design-tokens.json](testdata/design-tokens.json), a checked-in
...
the generated file is stale. Distances live in `src/ui/scale.ts`.
